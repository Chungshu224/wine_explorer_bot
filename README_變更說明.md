...
> **更新(第二版)**:這版的 `story_data.json` 已經合併了你上傳的檔案,裡面
> 補上了「薄酒萊 Beaujolais」故事線(`beaujolais_line`,《整串的祕密》,
> 6 個章節,主角圍繞 Morgon 跟二氧化碳浸漬法),以及另外三個章節(柯通丘、
> 夏布利、風土偵探各一個節點)新增的地圖深連結。這些都已經跑過完整性檢查
> (章節連結、badge 解鎖順序、chapter_order 一致性全部通過),放心套用。
> 薄酒萊會自動歸類到「布根地」大產區底下,跟夜丘、伯恩丘等排在同一層次
> 產區列表裡,不需要額外設定。
