...
def main():
    ...
    vocab_en = "\n".join("• " + v for v in content.get("vocab_en", []))
    vocab_fr = "\n".join("• " + v for v in content.get("vocab_fr", []))

    message = (
        "📅 " + datetime.now().strftime("%Y-%m-%d") + " 每日布根地聽力：" + topic + "\n\n"
        + "🇬🇧 " + content["en"] + "\n\n生字：\n" + vocab_en + "\n\n"
        + "🇫🇷 " + content["fr"] + "\n\n生字：\n" + vocab_fr
    )
